...
class DNN_Core:
    ...
    def update_parameters(self, parameters, gradients, alpha, reg_rate, m):
        ...
        for l in range(layers):
            #parameters["W" + str(l+1)] += (reg_rate/m) * parameters["W" + str(l+1)]
            parameters["W" + str(l+1)] -= alpha * gradients["dW" + str(l+1)]
            parameters["b" + str(l+1)] -= alpha * gradients["db" + str(l+1)]
        return parameters
    
    def feedback(self, X, T, parameters, alpha, reg_rate):
    ...
    def batchGD(self, X, Y, epochs, alpha, layer_dims, parameters, reg_rate, print_cost):
        util = Util()
        J_history = []
        Acc_history = []
        T = util.yEnc(Y)
        for i in range(epochs):
            A, gradients, parameters = self.feedback(X, T, parameters, alpha, reg_rate)
            if print_cost:
                if i % 100 == 0:
    ...
    def SGD(self, X, Y, epochs, alpha, layer_dims, parameters, batch_size, reg_rate, print_cost):
        util = Util()
        J_history = []
        Acc_history = []
        T = util.yEnc(Y)
...
class DNN:
    ...
    def fit(self, X, Y, epochs, alpha, layer_dims, batch_type, reg_rate):
        core = DNN_Core(self.layer_dims)
        parameters = core.initialise_parameters()
        if batch_type == 'full':
            A, gradients, parameters = core.batchGD(X, Y, epochs, alpha, layer_dims, parameters, reg_rate, True)
        else:
            if batch_type == 'mini_batch':
                batch_size = 512
            else:
                batch_size = 1
            A, gradients, parameters = core.SGD(X, Y, epochs, alpha, layer_dims, parameters, batch_size, reg_rate, True)
        return A, gradients, parameters
...
Yhat, gradients, parameters = deepNN.fit(X_train, Y_train, 1500, 0.1, layer_dims, 'mini_batch', 0.1)

accuracy = deepNN.accuracy(Yhat, Y_train)

Yhat_test = deepNN.predict(X_test, parameters)
